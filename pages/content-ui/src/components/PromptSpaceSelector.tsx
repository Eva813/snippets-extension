    ...
    <div className="relative border-b bg-white p-3" ref={dropdownRef}>
      ...
      {isOpen && (
        <div 
          ...
          className="absolute inset-x-0 top-full z-50 mt-1 max-h-[200px] overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg">
          {mySpaces.length > 0 && (
            <div>
              <div className="bg-gray-50 px-4 py-2 text-xs font-medium text-gray-500">{SECTION_TITLES.MY_WORKSPACES}</div>
              {mySpaces.map(space => (
                <button
                  key={space.id}
                  role="option"
          ...
          {sharedSpaces.length > 0 && (
            <div className={mySpaces.length > 0 ? 'border-t border-gray-100' : ''}>
              <div className="bg-gray-50 px-4 py-2 text-xs font-medium text-gray-500">{SECTION_TITLES.SHARED_WITH_ME}</div>
